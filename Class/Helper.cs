...
namespace BennysMotorworksRevamped
{
    ...
    public static class Helper
    {
        ...
        private const int MPDLCMapInitialDelayMs = 10000;
        private const int MPDLCMapRetryDelayMs = 1000;
        private const int MPDLCMapValidationDelayMs = 1000;
        private const int MPDLCMapContentSettleDelayMs = 2000;
        ...
        private static int _bennysMPContentTransitionStartTime = 0;
        ...
        private static bool IsEnhancedGameBuild()
        {
            try
            {
                return System.Diagnostics.Process.GetCurrentProcess().ProcessName.IndexOf("Enhanced", StringComparison.OrdinalIgnoreCase) >= 0;
            }
            catch
            {
                return false;
        ...
        private static int GetBennysInteriorId(out int referenceProbeInteriorId, out int workshopInteriorId)
        {
            ...
            workshopInteriorId = GetInteriorID(new Vector3(-211.798f, -1324.292f, 30.37535f));
        ...
        private static void ResetMPDLCMapLoadState(bool restoreStoryContent)
        {
            ...
            _bennysMPContentTransitionStartTime = 0;
        ...
        {
            Function.Call(Hash.REQUEST_COLLISION_AT_COORD, -211.798f, -1324.292f, 30.37535f);
        ...
        internal static void ProcessPendingMPDLCMapLoad()
        {
            ...
            if (Game.GameTime < MPDLCMapInitialDelayMs
                ...
                || Function.Call<bool>((Hash)IsLoadingScreenActiveHash)
                || Function.Call<bool>((Hash)IsInitialLoadingScreenActiveHash))
            {
                return;
            }
            ...
            try
            {
                ...
                if (!_bennysMPContentTransitionStarted)
                {
                    ...
                    if (existingLowriderSetActive && existingInteriorValid)
                    {
                        ActivateBennysInterior(existingInteriorId);
                        _pendingMPDLCMapLoad = false;
                        Logger.Log("LoadMPDLCMap: existing Lowrider map state detected; adopting Benny's interior without changing external map ownership. interiorId="
                    ...
                    Logger.Log("LoadMPDLCMap: " + editionName + " - invoking ON_ENTER_MP to register Online/Lowrider map content.");
                    Function.Call((Hash)OnEnterMpHash);
                    _bennysMPContentTransitionStarted = true;
                    _bennysMPContentTransitionStartTime = Game.GameTime;
                    Logger.Log("LoadMPDLCMap: " + editionName + " - ON_ENTER_MP returned successfully; waiting for the content change set to settle.");
                    return;
                }

                if (!_bennysMPContentTransitionSettled)
                {
                    if (Game.GameTime - _bennysMPContentTransitionStartTime < MPDLCMapContentSettleDelayMs
                        || AreContentChangeSetsPending())
                    {
                        return;
                    }
                    ...
                    _nextMPDLCMapLoadAttemptTime = 0;
                    _nextMPDLCMapDebugSnapshotTime = 0;
                    Logger.Log("LoadMPDLCMap: MP content change set settled; requesting the complete Benny's Lowrider sc1_02 sector.");
                }
                ...
                if (!_bennysStoryRestoreStarted)
                {
                    ...
                    if (Game.GameTime - _bennysInteriorRequestStartTime < MPDLCMapValidationDelayMs)
                    {
                        return;
                    }
                    ...
                    if (lowriderSetActive && validInterior)
                    {
                        _pendingMPDLCMapLoad = false;
                        Logger.Log("LoadMPDLCMap: Benny's Lowrider map registered successfully on "
                            + (IsEnhancedGameBuild() ? "Enhanced" : "Legacy")
                            + ". interiorId=" + interiorId
                            + " ready=" + interiorReady
                            + " IPL active=" + supermodIplActive
                    ...
                    {
                        Logger.Log("LoadMPDLCMap: timed out waiting for Benny's after MP registration. storyIpls=["
                            ...
                            + " ready=" + interiorReady + ". Restoring Story content.");
                        RestoreStoryContentState();
                    }
        ...
        public static void SetWorkshopCarModShopState(bool enabled)
        {
            try
            {
                int targetHandle = enabled && veh != null && veh.Exists() ? veh.Handle : 0;

                if (_carModShopVehicleHandle != 0 && _carModShopVehicleHandle != targetHandle)
                {
                    if (Function.Call<bool>(Hash.DOES_ENTITY_EXIST, _carModShopVehicleHandle))
                    {
                        Function.Call((Hash)SetVehicleInCarModShopHash, _carModShopVehicleHandle, false);
                    }
            ...
            {
                Logger.Log("Unable to update car mod shop vehicle state: " + ex.Message);
            }
        }

        public static void EnableWorkshopMenuMouseControls()
        {
            try
            {
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.Attack, true);
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.Aim, true);
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.LookLeftRight, true);
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.LookUpDown, true);
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.CursorAccept, true);
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.CursorCancel, true);
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.CursorX, true);
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.CursorY, true);
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.CursorScrollUp, true);
                Function.Call(Hash.ENABLE_CONTROL_ACTION, 0, (int)Control.CursorScrollDown, true);
            }
            catch (Exception ex)
            {
                Logger.Log("Unable to restore workshop mouse controls: " + ex.Message);
            }
        }

        public static void SetWorkshopPlayerControlSuppressed(bool enabled)
        {
            try
            {
                ...
                if (enabled)
                {
                    ...
                    _workshopPlayerControlSuppressionActive = true;
                    _workshopOwnsPlayerControlSuppression = Function.Call<bool>(Hash.IS_PLAYER_CONTROL_ON, player.Handle);
                    if (_workshopOwnsPlayerControlSuppression)
                    {
                        Function.Call(Hash.SET_PLAYER_CONTROL, player.Handle, false, 256);
                    }
                    return;
                }
        ...
        public static void SuspendKeys()
        {
            ...
            Game.DisableControlThisFrame(Control.VehiclePrevRadio);
            Game.DisableControlThisFrame(Control.VehicleNextRadioTrack);
            Game.DisableControlThisFrame(Control.VehiclePrevRadioTrack);
        ...
        private static void ReduceWorkshopCutsceneBlur()
        {
            if (scriptCam != null)
            {
                Function.Call(Hash.SET_CAM_MOTION_BLUR_STRENGTH, scriptCam.Handle, 0.35f);
            }

            Function.Call(Hash.TRIGGER_SCREENBLUR_FADE_OUT, 1250.0f);
        }
